...
export function Schedules() {
  ...
  // Daily time (12-hour safe picker)
  ...
  const [weeklyDays, setWeeklyDays] = useState<{[k: string]: boolean}>({ '1': true, '2': true, '3': true, '4': true, '5': true }) // Mon-Fri
  // Monthly day + time
  ...
  useEffect(() => {
    ...
    switch (mode) {
      ...
      case 'weekly': {
        ...
        const selected = Object.entries(weeklyDays).filter(([, v]) => v).map(([k]) => k)
      ...
      case 'custom':
      default:
        // Leave cron as-is for custom mode
        break
    }
  }, [manualCron, mode, everyN, hourlyMinute, dailyHour12, dailyMinute, dailyMeridiem, weeklyHour12, weeklyMinute, weeklyMeridiem, weeklyDays, monthlyDay, monthlyHour12, monthlyMinute, monthlyMeridiem])
          ...
          <div className="space-y-3">
            ...
            <RadioGroup value={mode} onValueChange={(v) => { setMode(v as any); handleBuilderInteraction() }} className="grid grid-cols-1 md:grid-cols-3 gap-2">
              ...
              <div className="flex items-center space-x-2">
                ...
                <Label htmlFor="mode-hourly" className="text-sm cursor-pointer">Hourly at minute</Label>
              </div>
              <div className="flex items-center space-x-2">
                <RadioGroupItem value="daily" id="mode-daily" />
                <Label htmlFor="mode-daily" className="text-sm cursor-pointer">Daily at time</Label>
              </div>
              <div className="flex items-center space-x-2">
                <RadioGroupItem value="weekly" id="mode-weekly" />
                <Label htmlFor="mode-weekly" className="text-sm cursor-pointer">Weekly on days</Label>
              </div>
              <div className="flex items-center space-x-2">
                <RadioGroupItem value="monthly" id="mode-monthly" />
                <Label htmlFor="mode-monthly" className="text-sm cursor-pointer">Monthly on day</Label>
              </div>
              <div className="flex items-center space-x-2">
                <RadioGroupItem value="custom" id="mode-custom" />
                <Label htmlFor="mode-custom" className="text-sm cursor-pointer">Custom cron</Label>
              </div>
            </RadioGroup>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3 items-start">
            ...
            <div>
              <div className="flex items-center justify-between">
                <Label>{manualCron ? 'Cron (manual)' : mode === 'custom' ? 'Cron (custom)' : 'Cron (generated)'}</Label>
                <label className="flex items-center gap-2 text-xs">
                  <Checkbox checked={manualCron} onCheckedChange={(v) => setManualCron(!!v)} />
                  Edit cron manually
                </label>
              </div>
              <Input value={cron} onChange={e => setCron(e.target.value)} placeholder="e.g. 0 6 * * *" disabled={!manualCron && mode !== 'custom'} />
